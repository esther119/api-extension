(async () => {
  ...
  console.log("content script get tip", tip);

  const nav = document.querySelector("li.devsite-nav-item");

  const tipWidget = createDomElement(`
      <button class="li.devsite-nav-item" popovertarget="tip-popover" popovertargetaction="show" style="padding: 0; border: none; background: none;>
  ...
  console.log("add popover", popover);
  nav.append(tipWidget);
})();

function createDomElement(html) {
  const dom = new DOMParser().parseFromString(html, "text/html");
  console.log("createDomElement fire element child", dom.body.firstElementChild);
